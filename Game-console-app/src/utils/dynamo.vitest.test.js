...
//   });
  ...
  it("createItem stores and returns the correct item with timezone", async () => {
    ddbMock.on(PutCommand).resolves({});

    const item = { id: "1", timezone: "14400" };

    const output = await createItem("weatherdata", item);

    expect(output).toEqual(item);
  });


  it("listItems and returns an array with timezone items", async () => {
    const mockItems = [{ id: "1", timezone: "14400" }, { id: "2", timezone: "7200" }];
    ddbMock.on(ScanCommand).resolves({Items: mockItems});

    const output = await listAllItems("weatherdata");
